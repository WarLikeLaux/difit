...
インストールされる `difit` skill は、変更内容の表示、特定の diff や PR へのインライン指摘の事前投入、ユーザーからの追加コメントへの対応を一つのワークフローで行います。
...
- **自動生成ファイル**: 自動生成されたコードはデフォルトで折りたたまれます。これには以下が含まれます：
  - ロックファイル (`package-lock.json`, `go.sum`, `Cargo.lock`, `Gemfile.lock` など)
