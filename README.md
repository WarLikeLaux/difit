...
| Agent inbox       | User replies plus `Accepted` and `Verify by agent` transitions enter a durable, acknowledged queue   | Feedback and requested actions reach the agent attached to that exact review and survive restarts            |
...
| Review workflow   | Threads show `Open`, `Accepted`, `Verify by agent`, `Ready for review`, and `Resolved`               | `Accepted` commands an agent edit, while externally assigned fixes remain `Open` until verification          |
