...
<body>
  ...
  <script>
    ...
    function reviewCard(review){const latest=review.threads[0];const state=reviewState(review);return '<article class="review-card '+state+'"><span class="state-rail" aria-hidden="true"></span><div class="review-main"><div class="review-title"><strong>'+esc(review.repositoryName)+'</strong><span class="slash">/</span><span class="branch">'+esc(review.label)+'</span><span class="state-label">'+esc(stateLabel(review))+'</span></div><div class="review-meta"><span>'+kindLabel(review)+'</span><time datetime="'+esc(review.updatedAt)+'" title="'+esc(new Date(review.updatedAt).toLocaleString())+'">'+(latest?'Last reply ':'Added ')+relativeTime(review.updatedAt)+'</time><span class="review-stats">'+stats(review)+'</span></div>'+(latest?'<div class="latest"><span class="author">'+esc(latest.lastAuthor||'Unknown')+':</span> '+esc(latest.lastMessage)+'</div>':'')+'</div><div class="actions">'+(review.viewerUrl?'<a class="primary" href="'+esc(review.viewerUrl)+'">Open</a>':'<span class="unavailable">Unavailable</span>')+(review.reviewUrl?'<a href="'+esc(review.reviewUrl)+'" target="_blank" rel="noreferrer">MR</a>':'')+'<button type="button" class="delete-review" data-delete-review="'+esc(review.id)+'" aria-label="Delete review" title="Delete review"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M3.5 4.5h9m-6.5 0V3h4v1.5m-5.5 0 .5 8h6l.5-8M6.75 6.5v4m2.5-4v4" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div></article>'}
