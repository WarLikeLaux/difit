...
<body>
  ...
  <script>
    const root=document.getElementById('reviews');const summary=document.getElementById('summary');let reviews=[];let allExpanded=false;
    ...
    function reviewCard(review,includeResolved=false){const threads=review.threads.filter((thread)=>includeResolved||thread.status!=='resolved');return '<section class="review"><div class="review-head"><div class="review-title"><strong>'+esc(review.repositoryName)+'</strong><span class="review-label">'+esc(kindLabel(review))+'</span><details class="review-details"><summary>Technical details</summary><code>'+esc(review.repositoryPath)+'</code><code>base '+esc(review.baseRef)+' / target '+esc(review.targetRef||'')+'</code></details></div><div class="actions">'+(review.viewerUrl?'<a href="'+esc(review.viewerUrl)+'">Open review</a>':'<span class="muted">Snapshot unavailable</span>')+(review.reviewUrl?'<a href="'+esc(review.reviewUrl)+'" target="_blank" rel="noreferrer">Open MR</a>':'')+'<button type="button" class="delete-review" data-delete-review="'+esc(review.id)+'">Delete review</button></div><div class="badges"><span class="badge '+(review.agentConnected?'connected':'offline')+'">Agent '+(review.agentConnected?'connected':'offline')+'</span>'+(review.pendingMessages?'<span class="badge queued">'+review.pendingMessages+' waiting</span>':'')+'<span class="badge open">Open '+review.counts.open+'</span><span class="badge accepted">Accepted '+review.counts.accepted+'</span><span class="badge to_verify">To verify '+review.counts.to_verify+'</span><span class="badge ready">Ready '+review.counts.ready+'</span></div></div>'+(threads.length?'<div class="threads">'+threads.map((thread)=>'<article class="thread"><div class="thread-top"><span class="thread-path">'+esc(thread.filePath)+':'+thread.line+'</span><span class="'+thread.status+'">'+esc(thread.status.replace('_',' '))+'</span></div><div class="message"><span class="muted">'+esc(thread.lastAuthor||'Unknown')+':</span> '+esc(thread.lastMessage)+'</div></article>').join('')+'</div>':'<div class="empty">No unresolved threads</div>')+'</section>'}
    function group(title,items){return '<section class="review-group"><div class="group-head"><h2 class="group-title">'+title+'</h2><span class="group-count">'+items.length+'</span></div>'+items.map((review)=>reviewCard(review)).join('')+'</section>'}
    function render(){const connected=reviews.filter((review)=>review.agentConnected);const waitingReviews=reviews.filter((review)=>!review.agentConnected&&review.pendingMessages>0);const waiting=waitingReviews.reduce((count,review)=>count+review.pendingMessages,0);summary.textContent=connected.length+' connected'+(waiting?' / '+waiting+' waiting':'');if(!reviews.length){root.innerHTML='<div class="empty">No reviews yet. Attach a Git checkout with difit.</div>';return}const connectedContent=connected.length?group('Connected now',connected):'<div class="group-head"><h2 class="group-title">Connected now</h2><span class="group-count">0</span></div><div class="connection-empty">No agents connected. Saved reviews are available below.</div>';const waitingContent=waitingReviews.length?group('Waiting for agent',waitingReviews):'';root.innerHTML=connectedContent+waitingContent+'<details class="all-reviews" data-all-reviews '+(allExpanded?'open':'')+'><summary>All reviews <span class="group-count">'+reviews.length+'</span></summary><div>'+reviews.map((review)=>reviewCard(review,true)).join('')+'</div></details>'}
    ...
    root.addEventListener('toggle',(event)=>{if(event.target.matches?.('[data-all-reviews]'))allExpanded=event.target.open},true);
